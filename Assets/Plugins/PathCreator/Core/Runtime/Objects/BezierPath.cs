...
namespace PathCreation
{
 ...
 centre + Vector3.right * width
			...
			perAnchorSpeeds = new List<float>() { 1, 1 };
		...
		public void AddSegmentToEnd(Vector3 anchorPos)
		{
			...
			perAnchorSpeeds.Add(perAnchorSpeeds[perAnchorSpeeds.Count - 1]);
		...
		public void AddSegmentToStart(Vector3 anchorPos)
		{
			...
			perAnchorSpeeds.Insert(0, perAnchorSpeeds[0]);
		...
		public void SplitSegment(Vector3 anchorPos, int segmentIndex, float splitTime)
		{
			...
			// Insert speed for new anchor
			int numSpeeds = perAnchorSpeeds.Count;
			int newSpeedIndex = (segmentIndex + 1) % numSpeeds;
			float speedPrev = perAnchorSpeeds[segmentIndex];
			float speedNext = perAnchorSpeeds[newSpeedIndex];
			float splitSpeed = Mathf.LerpAngle (speedPrev, speedNext, splitTime);
			perAnchorSpeeds.Insert(newSpeedIndex, splitSpeed);
		...
		public void DeleteSegment(int anchorIndex)
		{
			...
			if (NumSegments > 2 || !isClosed && NumSegments > 1)
			{
				...
				perAnchorSpeeds.RemoveAt(anchorIndex / 3);
		...
		public float GetAnchorSpeed(int anchorIndex)
		{
			if (perAnchorSpeeds.Count == 0 && perAnchorNormalsAngle.Count > 0)
			{
				Debug.LogError($"Missing anchor speeds. Fixing...");

				perAnchorSpeeds = new List<float>(perAnchorNormalsAngle.Count);
				for (int i = 0; i < perAnchorNormalsAngle.Count; i++)
				{
					perAnchorSpeeds.Add(1f);
				}
			}

			return perAnchorSpeeds[anchorIndex];
		}

		public void SetAnchorSpeed(int anchorIndex, float newSpeed)
		{
			perAnchorSpeeds[anchorIndex] = newSpeed;
		}
