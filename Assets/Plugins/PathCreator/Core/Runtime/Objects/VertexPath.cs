...
namespace PathCreation
{
	...
	public class VertexPath
	{
		...
		public readonly float[] localSpeeds;
		...
		public VertexPath(BezierPath bezierPath, Transform transform, float maxAngleError = 0.3f, float minVertexDst = 0f, float maxVertexDst = 0f) :
			this(bezierPath, VertexPathUtility.SplitBezierPathByAngleError(bezierPath, maxAngleError, minVertexDst, maxVertexDst, VertexPath.accuracy), transform)
		{ }
		...
		VertexPath(BezierPath bezierPath, VertexPathUtility.PathSplitData pathSplitData, Transform transform)
		{
			...
			localSpeeds = new float[numVerts];
			...
			for (int anchorIndex = 0; anchorIndex < pathSplitData.anchorVertexMap.Count - 1; anchorIndex++)
			{
				...
				float startSpeed = bezierPath.GetAnchorSpeed(anchorIndex);
				float endSpeed = bezierPath.GetAnchorSpeed(nextAnchorIndex);

				int startVertexIndex = pathSplitData.anchorVertexMap[anchorIndex];
				int endVertexIndex = pathSplitData.anchorVertexMap[anchorIndex + 1];

				float startTime = times[startVertexIndex];
				float endTime = times[endVertexIndex];
				...
				for (int i = startVertexIndex; i <= endVertexIndex; i++)
				{
					...
					localSpeeds[i] = Mathf.Lerp(startSpeed, endSpeed, progress);
				}
			}
		}
