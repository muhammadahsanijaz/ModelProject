...
namespace PathCreation.Utility
{
	public static class VertexPathUtility
	{
		...
		public static PathSplitData SplitBezierPathByAngleError(BezierPath bezierPath, float maxAngleError, float minVertexDst, float maxVertexDst, float accuracy)
		{
			...
			for (int segmentIndex = 0; segmentIndex < bezierPath.NumSegments; segmentIndex++)
			{
				...
				for (float t = increment; t <= 1; t += increment)
				{
					...
					if ((angleError > maxAngleError && dstSinceLastVertex >= minVertexDst) || isLastPointOnPath || dstSinceLastVertex > maxVertexDst)
					{
