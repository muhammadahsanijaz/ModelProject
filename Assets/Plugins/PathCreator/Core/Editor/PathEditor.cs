	...
	public class PathEditor : Editor
	{
				...
				if (data.showPathOptions)
				{
					...
					if (handleIndexToDisplayAsTransform != -1)
					{
						...
						using (new EditorGUI.IndentLevelScope())
						{
							...
							float currentSpeed = creator.bezierPath.GetAnchorSpeed(anchorIndex);
							float newSpeed = EditorGUILayout.FloatField ("Speed", currentSpeed);
							if (newSpeed != currentSpeed)
							{
								Undo.RecordObject(creator, "Set Speed");
								creator.bezierPath.SetAnchorSpeed(anchorIndex, newSpeed);
							}
						}
					}
		...
		void DrawVertexPathInspector()
		{
			...
			if (data.showVertexPathOptions)
			{
				using (var check = new EditorGUI.ChangeCheckScope())
				{
					...
					data.vertexPathMinVertexSpacing = EditorGUILayout.Slider(new GUIContent("Min Vertex Distance"), data.vertexPathMinVertexSpacing, 0, 2);
					data.vertexPathMaxVertexSpacing = EditorGUILayout.Slider(new GUIContent("Max Vertex Distance"), data.vertexPathMaxVertexSpacing, data.vertexPathMinVertexSpacing, 20f);
		...
		void DrawVertexPathSceneEditor()
		{
			...
			for (int i = 0; i < creator.path.NumPoints; i++)
			{
				...
				{
					float speedAverage = (creator.path.GetSpeed(i) + creator.path.GetSpeed(nextIndex)) * 0.5f;
					Handles.color = Color.Lerp(globalDisplaySettings.vertexPathLowSpeed, globalDisplaySettings.vertexPathHighSpeed, speedAverage);
		...
		void DrawHandle(int i)
		{
			...
			{
				handleColours.defaultColour = Color.Lerp(globalDisplaySettings.anchorLowSpeedColor, globalDisplaySettings.anchorHighSpeedColor, bezierPath.GetAnchorSpeed(i / 3));
			}
